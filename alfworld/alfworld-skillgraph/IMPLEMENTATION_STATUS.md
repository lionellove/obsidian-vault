...
- DeepSeek JSON-mode meta calls include role-specific example output shapes and recover from provider-side empty `content` with at most three audited attempts. Retry output budgets rise deterministically from 2,048 to 4,096 to 8,192 tokens; raw responses, finish reasons, normalized usage, and a terminal generation error are persisted so repeated empties are diagnosable without exposing credentials.
...
`pytest` is not installed in this environment. `python -m py_compile`, 88 direct target `test_*` functions (including empty-content retry/audit persistence, atomic-state, checkpoint-journal, evolution-hash, blind-packet, reject-s0, pricing, WSL logic-path resolution, and earlier P0/S0/metrics/pipeline/CLI suites), and the existing 8 `run_alf_bench` unittest cases pass. A run remains `awaiting_human_audit` until a complete blinded eight-item score packet is submitted for every anonymous candidate; only then can it become `completed` (with expert-plan evidence explicitly deferred and cost usage complete).
